...
#pragma omp simd
	...
	Vector3 Vector3::ProjectVector(const Vector3 & one, const Vector3 & two)
	{
#ifdef RZ_PROFILING
		ProfilerObj autoProfiler(RZ__FUNCTION__, PROFILE_EVENTS::PROF_GAME);
#endif

		const auto num = (one | two);
		const auto denom = two.SqrMagnitude();

		return two * (num / denom);
	}

	Vector3 Vector3::ProjectVectorPerpendicular(const Vector3 & one, const Vector3 & two)
	{
#ifdef RZ_PROFILING
		ProfilerObj autoProfiler(RZ__FUNCTION__, PROFILE_EVENTS::PROF_GAME);
#endif

		return one - ProjectVector(one, two);
	}

	float Vector3::DotProduct(const Vector3 & dotLhs, const Vector3 & dotRhs)
	{
#ifdef RZ_PROFILING
		ProfilerObj autoProfiler(RZ__FUNCTION__, PROFILE_EVENTS::PROF_GAME);
#endif
	...
	Vector3 Vector3::CrossProduct(const Vector3 & crossLhs, const Vector3 & crossRhs)
	{
#ifdef RZ_PROFILING
		ProfilerObj autoProfiler(RZ__FUNCTION__, PROFILE_EVENTS::PROF_GAME);
#endif

		return crossLhs ^ crossRhs;
	}

	float Vector3::TripleProduct(const Vector3 & dotV, const Vector3 & crossLhs, const Vector3 & crossRhs)
	{
#ifdef RZ_PROFILING
		ProfilerObj autoProfiler(RZ__FUNCTION__, PROFILE_EVENTS::PROF_GAME);
#endif

		return dotV | (crossLhs ^ crossRhs);
	}
