...
void GLAPIENTRY GLDebugMessageProc(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam)
{
    (void)id;
    (void)length;
    (void)userParam;
...
wglSwapIntervalEXT_Func wglSwapIntervalEXT = nullptr;
...
DeviceManager::CreateWindowDeviceAndSwapChain(const DeviceCreationParameters& params, LPWSTR title)
{
    ...
    if (glewInit() != GLEW_OK)
    {
        MessageBox(NULL, L"Failed to initialize GLEW library", m_WindowTitle.c_str(), MB_OK | MB_ICONERROR);
        return E_FAIL;
    }
    ...
    if (params.enableDebugRuntime)
    {
        ...
        glDebugMessageCallback(GLDebugMessageProc, nullptr);
    }
    ...
    while (WM_QUIT != msg.message)
    {
        ...
        else
        {
            ...
            if(m_GLcontext && GetWindowState() != kWindowMinimized)
            {
                ...
                Render(); 
                if(wglSwapIntervalEXT) wglSwapIntervalEXT(m_SyncInterval);
                SwapBuffers(m_DC);
                Sleep(0);
            }
            else
