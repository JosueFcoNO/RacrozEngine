	...
	Vector<StrPtr<Animation>> SkinnedModel::LoadAnimations(char * filePath)
	{
		...
		if (Scene->HasAnimations())
		{
			...
			for (uint32 i = 0; i < Scene->mNumAnimations; ++i)
			{
				...
				TempAnim->m_Duration = CastStatic<float>(CurrentAnim->mDuration);
				TempAnim->m_TicksPerSecond = CastStatic<float>(CurrentAnim->mTicksPerSecond);
				...
				if (RootNameTrans[0] == RootNameScale[0] && RootNameTrans[0] == RootNameRot[0])
				{
					...
					for (uint32 k = 0; k < CurrentAnim->mChannels[0]->mNumPositionKeys; ++k)
					{
						...
						TempFrame.m_Time = CastStatic<float>(CurrentAnim->mChannels[0]->mPositionKeys[k].mTime);
						...
						TempFrame.m_Rotation.m_v.m_x = CurrentAnim->mChannels[1]->mRotationKeys[k].mValue.x;
						TempFrame.m_Rotation.m_v.m_y = CurrentAnim->mChannels[1]->mRotationKeys[k].mValue.y;
						TempFrame.m_Rotation.m_v.m_z = CurrentAnim->mChannels[1]->mRotationKeys[k].mValue.z;
				...
				for (; o < CurrentAnim->mNumChannels; ++o)
				{
					...
					for (uint32 k = 0; k < CurrentAnim->mChannels[o]->mNumPositionKeys; ++k)
					{
						...
						TempFrame.m_Time = CastStatic<float>(TempChannel->mPositionKeys[k].mTime);
