...
namespace rczEngine
{
	...
	void TerrainPatch::Render(Gfx::GfxCore * gfx, ResVault * res)
	{
	...
	void TerrainPatch::GenerateVertexBuffer(MeshPlane* left, MeshPlane * right, MeshPlane * down, MeshPlane * rightDown)
	{
		...
		for (int32 i = 0; i < 512; ++i)
		{
			...
			int shet = i / LeftMul; ///Get the row for the vertex.

			if (i % LeftMul == 0) ///If there's a corresponding vertex.
			{
				PrevVertex1 = &left->m_VertexBuffer.GetVertex((shet+1) * (LeftSize)-1); ///Get the vertex and save it as the last corresponding vertex.
				PrevVertex1->VertexPosition.m_z -= 511.0f;
				PrevVertex1->TextureCoordinates.m_x = shet;
				PrevVertex1->TextureCoordinates.m_y = shet;

				TempVertex1 = *PrevVertex1; ///Create the new vertex.
			}
			else ///If there's no corresponding vertex, we create one interpolating.
			{
				Next = &left->m_VertexBuffer.GetVertex((shet + 1) * (LeftSize - 1));
			...
			shet = i / RightMul; ///Get the row for the vertex.

			if (i % RightMul == 0) ///If there's a corresponding vertex.
			{
				PrevVertex2 = &right->m_VertexBuffer.GetVertex((shet) * (RightSize)); ///Get the vertex and save it as the last corresponding vertex.
			...
			{
				Next = &right->m_VertexBuffer.GetVertex((shet + 1) * (RightSize));
		...
		//	if (i % LeftMul == 0) ///If there's a corresponding vertex.
		//	{
		//		PrevVertex1 = &left->m_VertexBuffer.GetVertex((shet)+(LeftSize - 1)*LeftSize); ///Get the vertex and save it as the last corresponding vertex.
		//		TempVertex1 = *PrevVertex1; ///Create the new vertex.
		//		m_Mesh.AddVertex(TempVertex1); ///Save the vertex;
		...
		//	else ///If there's no corresponding vertex, we create one interpolating.
		//	{
		//		Next = &left->m_VertexBuffer.GetVertex((shet + 1) + (LeftSize - 1)*LeftSize);
		...
		//	shet = i / DownMul; ///Get the row for the vertex.

		//	if (i % DownMul == 0) ///If there's a corresponding vertex.
		//	{
		//		PrevVertex2 = &down->m_VertexBuffer.GetVertex(shet); ///Get the vertex and save it as the last corresponding vertex.
		//		TempVertex2 = *PrevVertex2; ///Create the new vertex.
		//		m_Mesh.AddVertex(TempVertex2); ///Save the vertex;
		...
		//	else ///If there's no corresponding vertex, we create one interpolating.
		//	{
		//		Next = &down->m_VertexBuffer.GetVertex((shet + 1));
