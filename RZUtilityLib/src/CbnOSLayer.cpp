...
namespace rczEngine
{
	...
	void OSLayer::InitOSLayer(InstanceHandle handle)
	{
		...
		ShowWindow(window.GetHandle(), 1);
		UpdateWindow(window.GetHandle());
	}

	Window& OSLayer::CreateCbnWindow(const String& windowID, int x, int y, int width, int height, long windowType)
	{
		auto handle = CreateWindowA(m_WindowClass.c_str(), windowID.c_str(), windowType, x, y, width, height, nullptr, nullptr,  m_Instance, nullptr);
