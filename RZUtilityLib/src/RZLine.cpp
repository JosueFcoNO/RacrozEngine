...
namespace rczEngine
{
	bool Line::TestPoint(const Vector3 & point)
	{
		...
		const auto denom = (m_PointTwo.m_x - m_PointOne.m_x);

		if (denom.SqrDistance == 0.0f) return false;

		auto U = (point - m_PointOne) / denom;
	...
	{
		auto p = m_PointOne + U*(m_PointOne - m_PointTwo);
