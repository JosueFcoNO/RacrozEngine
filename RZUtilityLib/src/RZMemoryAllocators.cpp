...
namespace rczEngine
{
	...
	/////////////////////////////////////////////////////
	template <class T>
	void MemAllocator::deallocate(T* ptr, SIZE_T num)
	{
		for (int i = 0; i < num; ptr++)
		{
			free(ptr);
		}
	}
	...
	template<class T>
	T* MemAllocator::allocate(SIZE_T num)
	{
		T* Temp = malloc(sizeof(T)*num);
		return Temp;
	}
	...
	template<class T>
	T * StackAllocator::allocate(SIZE_T num)
	{
		///Check if there's enough bytes for the allocation, if not return NULL
	...
	template<class T>
	void StackAllocator::deallocate(T * ptr, SIZE_T num)
	{
		m_ByteIndex -= num * sizeof(T);
	}
