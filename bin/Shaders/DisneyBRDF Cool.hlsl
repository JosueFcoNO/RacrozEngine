...
PS_Output PS_Main(PS_Input Input)
{
    ...
    float4 albedoColor = AlbedoTexture.Sample(LinearWrapSampler, Input.Texcoord) * (1.0f - g_OverrideAlbedo);

    //Sample the original normal value from the texture
    float3 normal = Decode(NormalMRTexture.Sample(LinearWrapSampler, Input.Texcoord).xy) * (1.0f - g_OverrideNormal);

    if (albedoColor.w == 0)
    {
        psout.PBR = float4(albedoColor.xyz, 1.0f);
    ...
    float metallic = NormalMRTexture.Sample(LinearWrapSampler, Input.Texcoord).z * (1.0f - g_OverrideMetallic);
    float roughness = NormalMRTexture.Sample(LinearWrapSampler, Input.Texcoord).w * (1.0f - g_OverrideRoughness);
   ...
   //Iluminación
    float4 pos = PositionTexture.SampleLevel(LinearWrapSampler, Input.Texcoord, 0.0f);
   ...
   //de las propiedades, que colores tienen elos metales.
    ...
    //////////////////////////////
    float3 envFresnel = Specular_F_Roughness(realSpecularColor.xyz, roughness*roughness, normal.xyz, viewDir).xyz;
   ...
   ///Bidirectional Reflection Distribution Function
    ...
    float mipIndex = roughness * 7.0f; //roughness * 9.0f;
    ...
    float3 Irradiance = saturate(EnviromentCube.SampleLevel(LinearWrapSampler, normal.xyz, 9.0f).xyz);
    Irradiance = pow(Irradiance, 2.2f);

    Irradiance *= 1.0f - metallic;

    float2 envBRDF = BRDFLUT.SampleLevel(LinearClampSampler, float2(saturate(dot(normal, viewDir)), clamp(roughness, 0.05f, 0.95f)), 0.0f).xy;

    float4 FinalColor = float4((albedoColor.xyz * Irradiance * Kd * g_AmbientLightIntensity) +
                                ((Enviroment) * (envFresnel * envBRDF.x + envBRDF.y) * g_ReflectionIntensity) +
                                (Light1 * (1.0f - metallic) * g_LightIntensity),
        1);

    //float SSAO = 1.0f - SSAOTex.Sample(LinearSampler, Input.Texcoord).r;
    //SSAO = pow(SSAO, 2.2f);

    FinalColor *= albedoColor.w * albedoColor.w;
