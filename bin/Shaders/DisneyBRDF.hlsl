...
PS_Output PS_Main(PS_Input Input)
{
    ...
    float4 albedoColor = AlbedoTexture.Sample(LinearSampler, Input.Texcoord) * (1.0f - g_OverrideAlbedo);
    //Sample the original normal value from the texture
    float3 normal = Decode(NormalMRTexture.Sample(LinearSampler, Input.Texcoord).xy) * (1.0f - g_OverrideNormal);
    //normal = NormalMRTexture.Sample(LinearSampler, Input.Texcoord).xyz;
    if (albedoColor.w == 0)
    {
    ...
    float metallic = NormalMRTexture.Sample(LinearSampler, Input.Texcoord).z * (1.0f - g_OverrideMetallic);
    float roughness = NormalMRTexture.Sample(LinearSampler, Input.Texcoord).w * (1.0f - g_OverrideRoughness);
   ...
   //Iluminación
    float4 pos = PositionTexture.SampleLevel(LinearSampler, Input.Texcoord, 0.0f);
   ...
   ///Bidirectional Reflection Distribution Function
    ...
    float4 FinalColor = float4((albedoColor.xyz * Irradiance * g_AmbientLightIntensity) * (1.0f - metallic) +
