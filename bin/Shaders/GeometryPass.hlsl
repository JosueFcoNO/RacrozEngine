...
PS_OUTPUT PS_Main(PS_Input frag) : SV_TARGET
{
    ...
    output.Color = AlbedoTexture.Sample(Sampler_, tex);
    ...
    output.Normal = float4(Encode(NormalFinal), MetallicTexture.Sample(Sampler_, tex).x, RoughnessTexture.Sample(Sampler_, tex).x);
    //output.Normal = float4((frag.normal), RoughnessTexture.Sample(Sampler_, tex).x);
