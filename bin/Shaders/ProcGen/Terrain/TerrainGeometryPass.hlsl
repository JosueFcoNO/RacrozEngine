...
PS_OUTPUT PS_Main(PS_Input frag) : SV_TARGET
{
    ...
    output.Normal = float4(Encode(frag.normal), MetallicTexture.Sample(Sampler_, tex).x, RoughnessTexture.Sample(Sampler_, tex).x);
    //output.Color = float4(frag.normal, 0.0f);
