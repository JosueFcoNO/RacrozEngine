...
float4 CompositingPS(FullScreenQuadOutput IN): SV_Target
{
    ...
    float indirectConfidence = g_EnableIndirectDiffuse ? t_IndirectConfidence[IN.position.xy].x : 0;
    ...
    float diffuse = shadow * NdotL;
    float ambient = lerp(1, indirectDiffuse.a, indirectConfidence);
    indirectDiffuse.rgb = pow(max(0, indirectDiffuse.rgb), 0.7);

    float3 result = albedo.rgb * (g_LightColor.rgb * diffuse + g_AmbientColor.rgb * ambient + indirectDiffuse.rgb) 
