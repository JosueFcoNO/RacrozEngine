...
TextureCube IrradianceCube : register(t6);

Texture2D ShadowMapTex : register(t7);
...
PS_Output PS_Main(PS_Input Input)
{
    ...
    float4 albedoColor = AlbedoTexture.Sample(LinearSampler, Input.tex0) * (1.0f - g_OverrideAlbedo);
    float3 normalO = mul(Input.TBN, NormalTexture.Sample(LinearSampler, Input.tex0).xyz);
    float3 normal = normalO.xyz * (1.0f - g_OverrideNormal);
    float3 Tangents = Input.TBN[0];
    float3 Binormals = Input.TBN[1];


    albedoColor = pow(albedoColor, 2.2f);

    float metallic = MetallicTexture.Sample(LinearSampler, Input.tex0) * (1.0f - g_OverrideMetallic);
    float roughness = RoughnessTexture.Sample(LinearSampler, Input.tex0) * (1.0f - g_OverrideRoughness);
   ...
   //Iluminación
    float4 pos = float4(Input.wpos, 1); //PositionTexture.Sample(LinearSampler, Input.Texcoord);
   ...
   ///Bidirectional Reflection Distribution Function
    ...
    for (int i = 0; i < g_LightNumber; ++i)
    {
        ...
        {
            //Light1 += (DisneyBRDF(albedoColor.xyz, spec, normal, roughness, lightDir, viewDir, Tangents.xyz, Binormals.xyz, diffuse, g_Lights[i].LightColor.xyz) / 1) * SpotLightDist;
            Light1 += ComputeLight(albedoColor.xyz, realSpecularColor, normal, roughness, g_Lights[i].LightPosition.xyz, g_Lights[i].LightColor.xyz, lightDir, viewDir, metallic);
    ...
    float mipIndex = roughness * 9.0f; 
    float3 reflectVector = reflect(-viewDir, normal.xyz);

    float4 envVector = float4(reflectVector.xyz, mipIndex);

    float3 Enviroment = EnviromentCube.SampleLevel(LinearSampler, envVector.xyz, mipIndex+0.0f).xyz;
    //Enviroment = pow(Enviroment, 2.2f);

    float3 Irradiance = EnviromentCube.SampleLevel(LinearSampler, envVector.xyz, 8.0f).xyz;

    float3 mixenvIrr = lerp(Enviroment, Irradiance, roughness * roughness);
    float2 envBRDF = BRDFLUT.Sample(LinearSampler, float2(roughness, dot(normal, viewDir))).xy;

    float4 FinalColor = float4((Light1 * g_LightIntensity * 1.0f) + (albedoColor.xyz *  Irradiance * g_AmbientLightIntensity) * 1.00f +
                                ((Enviroment) * (envFresnel) * g_ReflectionIntensity) * 1.0f
                               , 1); 

    //float SSAO = 1.0f - SSAOTex.Sample(LinearSampler, Input.Texcoord).r;

    FinalColor *= albedoColor.w;
    FinalColor *= ShadowMapTex.Sample(LinearSampler, Input.tex0).r;
