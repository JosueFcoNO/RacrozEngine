...
void GenerateMesh(__global struct GenerationData* GenData, __global char* GradIndex, __global struct Vector2* GradientVector, __global struct Vertex* OutputVertex)
{
	...
	double xPos = (xxPos / 512.0) / 50.0 + 1.0f / 50.0f * GenData[0].xCell;
	double yPos = (yxPos / 512.0) / 50.0 + 1.0f / 50.0f * GenData[0].yCell;
	...
	Nu.VertexPosition.x = OutputVertex[i].VertexPosition.x;
	Nu.VertexPosition.y = OutputVertex[i].VertexPosition.y;
	Nu.VertexPosition.z = OutputVertex[i].VertexPosition.z;

	Normalize(&Nu.VertexPosition);
